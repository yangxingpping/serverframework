...
void SWebSocketManagerPair::PushWebSocketClientResponse(ResponseType type, SessionType sid, const char* msg, size_t len)
{
...
void SWebSocketManagerPair::PushHttpClientResponse(ResponseType type, SessionType sid, const char* msg, size_t len)
{
	if (sid == INVALID_SESSION_ID || !msg)
	{ //record some error message
		return;
...
void SWebSocketManagerPair::PushHttpsClientResponse(ResponseType type, SessionType sid, const char* msg, size_t len)
{
	if (sid == INVALID_SESSION_ID || !msg)
	{ //record some error message
		return;
...
bool SWebSocketManagerPair::InitHttpManagerPair(std::string ip, uint16_t port, std::shared_ptr<MessageQueueManager> msgqueue)
{
	bool bret = false;
	...
	_recvHttpThread = std::make_shared<std::thread>(&SWebSocketManagerPair::RecvHttpDataThreadFunc, this);
	if (_logger)
	{
		bret = true;
	}
...
bool SWebSocketManagerPair::InitHttpsManagerPair(std::string ip, uint16_t port, std::shared_ptr<MessageQueueManager> msgqueue)
{
	bool bret = false;
	_httpsIP = ip;
	_httpsPort = port;
	_msgQueueP = msgqueue;
	_logger = spdlog::get(LOGGER_NAME);
	_recvHttpsThread = std::make_shared<std::thread>(&SWebSocketManagerPair::RecvHttpsDataThreadFunc, this);
	if (_logger)
	{
		bret = true;
	}
...
bool SWebSocketManagerPair::InitWsManagerPair(std::string ip, uint16_t port, std::shared_ptr<MessageQueueManager> msgqueue)
{
	bool bret = false;
	_wsIP = ip;
	_wsPort = port;
	_msgQueueP = msgqueue;
	_logger = spdlog::get(LOGGER_NAME);
	_recvWsThread = std::make_shared<std::thread>(&SWebSocketManagerPair::RecvWsDataThreadFunc, this);
	if (_logger)
	{
		bret = true;
	}
...
void SWebSocketManagerPair::RecvHttpDataThreadFunc()
{
...
void SWebSocketManagerPair::RecvHttpsDataThreadFunc()
{
	using namespace uWS;
	using WebSocketBehavior = typename uWS::TemplatedApp<false>::WebSocketBehavior;
	using WebSocket = uWS::WebSocket<false, true>;
...
void SWebSocketManagerPair::RecvWsDataThreadFunc()
{
	using WebSocketBehavior = typename uWS::TemplatedApp<false>::WebSocketBehavior;
	using WebSocket = uWS::WebSocket<false, true>;
	using HttpRequest = uWS::HttpRequest;
