...
bool HttpManagerPair::InitHttpsManagerPair(std::string ip, uint16_t port, std::shared_ptr<MessageQueueManager> msgqueue)
{
	bool bret = false;
	_httpsIP = ip;
	_httpsPort = port;
	_msgQueueP = msgqueue;
	_logger = spdlog::get(LOGGER_NAME);
	_recvHttpsThread = std::make_shared<std::thread>(&HttpManagerPair::RecvHttpsDataThreadFunc, this);
	if (_logger)
	{
		bret = true;
	}
...
bool HttpManagerPair::InitWsManagerPair(std::string ip, uint16_t port, std::shared_ptr<MessageQueueManager> msgqueue)
{
	bool bret = false;
	_wsIP = ip;
	_wsPort = port;
	_msgQueueP = msgqueue;
	_logger = spdlog::get(LOGGER_NAME);
	_recvWsThread = std::make_shared<std::thread>(&HttpManagerPair::RecvWsDataThreadFunc, this);
	if (_logger)
	{
		bret = true;
	}
...
bool HttpManagerPair::InitWssManagerPair(std::string ip, uint16_t port, std::shared_ptr<MessageQueueManager> msgqueue)
{
	bool bret = false;
	_wssIP = ip;
	_wssPort = port;
	_msgQueueP = msgqueue;
	_logger = spdlog::get(LOGGER_NAME);
	_recvWssThread = std::make_shared<std::thread>(&HttpManagerPair::RecvWssDataThreadFunc, this);
	if (_logger)
	{
		bret = true;
	}
...
void HttpManagerPair::RecvHttpsDataThreadFunc()
{
	using namespace uWS;
	using WebSocketBehavior = typename uWS::TemplatedApp<false>::WebSocketBehavior;
	using WebSocket = uWS::WebSocket<false, true>;
...
void HttpManagerPair::RecvWsDataThreadFunc()
{
	using WebSocketBehavior = typename uWS::TemplatedApp<false>::WebSocketBehavior;
	using WebSocket = uWS::WebSocket<false, true>;
	using HttpRequest = uWS::HttpRequest;
...
void HttpManagerPair::RecvWssDataThreadFunc()
{
