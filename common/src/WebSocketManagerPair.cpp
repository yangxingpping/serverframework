...
bool WebSocketManagerPair::InitHttpManagerPair(std::string ip, uint16_t port, std::shared_ptr<MessageQueueManager> msgqueue)
{
	bool bret = false;
	...
	_recvHttpThread = std::make_shared<std::thread>(&WebSocketManagerPair::RecvHttpDataThreadFunc, this);
	if (_logger)
	{
		bret = true;
	}
...
void WebSocketManagerPair::RecvHttpDataThreadFunc()
{
...
void WebSocketManagerPair::RecvHttpsDataThreadFunc()
{
	using namespace uWS;
	using WebSocketBehavior = typename uWS::TemplatedApp<false>::WebSocketBehavior;
	using WebSocket = uWS::WebSocket<false, true>;
...
void WebSocketManagerPair::RecvWssDataThreadFunc()
{
